...
public class DemonicLightRenderer implements BlockEntityRenderer<DemonicLightTile> {
    ...
    public void render(DemonicLightTile lightTile, float v, PoseStack matrixStack, MultiBufferSource iRenderTypeBuffer, int i, int i1) {
        ...
        if (!Minecraft.getInstance().isPaused()) {
            world.addParticle(DarkGlowParticleData.createData(new ParticleColor(RandColor(lightTile.red,rand), RandColor(lightTile.green,rand), RandColor(lightTile.blue,rand)), 0.25F, 0.9F, 36), (double)pos.getX() + 0.5 + ParticleUtil.inRange(-0.1, 0.1), (double)pos.getY() + 0.5 + ParticleUtil.inRange(-0.1, 0.1), (double)pos.getZ() + 0.5 + ParticleUtil.inRange(-0.1, 0.1), 0.0, 0.0, 0.0);
        }
    }

    int RandColor(int max,Random rand){
        int x = (max/3) + 1;
