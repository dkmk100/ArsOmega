...
public class HexedItem extends Item implements ICasterTool {
    ...
    public InteractionResultHolder<ItemStack> use(Level worldIn, Player playerIn, InteractionHand handIn) {
        ...
        InteractionResultHolder<ItemStack> cast = cast(worldIn,playerIn,handIn, Component.translatable("ars_nouveau.wand.invalid"),caster.getSpell(),caster.getColor().toWrapper());
        playerIn.getItemInHand(handIn).shrink(1);
        return cast;
    }

    InteractionResultHolder<ItemStack> cast(Level worldIn, Player playerIn, InteractionHand handIn, Component invalidMessage, Spell spell, ParticleColor.IntWrapper color){
        ItemStack stack = playerIn.getItemInHand(handIn);
        if (worldIn.isClientSide) {
            ...
            SpellResolver resolver = new SpellResolver((new SpellContext(worldIn,spell, playerIn)).withColors(color.toParticleColor()));
            ...
            if (result instanceof BlockHitResult && worldIn.getBlockEntity(((BlockHitResult)result).getBlockPos()) instanceof ScribesTile) {
                ...
                resolver.onResolveEffect(worldIn,new EntityHitResult(playerIn));
