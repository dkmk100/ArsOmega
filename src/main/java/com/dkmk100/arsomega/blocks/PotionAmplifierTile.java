...
public class PotionAmplifierTile extends TileEntity implements ITickableTileEntity {
    int timeMixing;
    boolean isMixing;
    boolean hasMana;

    public PotionAmplifierTile() {
        super(RegistryHandler.PotionAmplifierType.get());
    }
    ...
    public List<EffectInstance> getBuffedEffects(List<EffectInstance> originals) {
        ...
        for (EffectInstance instance:originals) {
            ...
            newEffects.add(new EffectInstance(instance.getEffect(), instance.getDuration(), Math.min(instance.getAmplifier()+1,20)));
        }
        return newEffects;
    }
